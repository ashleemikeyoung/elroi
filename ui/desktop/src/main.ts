...
const MENU_TRANSLATIONS_ZH_CN: Record<string, string> = {
  ...
  'Focus ElRoi Window': '聚焦 ElRoi 窗口',
  'Quick Launcher': '快速启动器',
  'Always on Top': '窗口置顶',
  'Toggle Navigation': '切换导航',
  'About ElRoi': '关于 ElRoi',
...
async function appMain() {
  ...
  const appMenu = menu?.items.find((item) => item.label === 'ElRoi' || item.label === 'Goose');
  if (appMenu?.submenu) {
  ...
  if (fileMenu?.submenu) {
    ...
    if (shortcuts.focusWindow) {
      fileMenu.submenu.append(
        new MenuItem({
          label: menuT('Focus ElRoi Window'),
  ...
  if (menu && process.platform !== 'darwin') {
    ...
    if (helpMenu.submenu) {
      ...
      // Create the About ElRoi menu item with a submenu
      const aboutGooseMenuItem = new MenuItem({
        label: menuT('About ElRoi'),
        submenu: Menu.buildFromTemplate([]), // Start with an empty submenu for About
      });

      // Add the Version menu item (display only) to the About ElRoi submenu
